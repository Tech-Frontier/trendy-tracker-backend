...
public class CompanyService {
  ...
  public Company updateCategory(String companyName, CompanyCategory category) {
    ...
    var company = companyRepository.updateCategory(
      findCompany.getCompany_name(),
      category
    );
  ...
  public Company updateGroup(String companyName, String group) {
    var findCompany = findCompany(companyName);
    Company company = companyRepository.updateGroup(
      findCompany.getCompany_name(),
      group
    );
  ...
  public List<CompanyInfo> getCompanyList() throws NoResultException {
    ...
    if (companyAll.size() == 0) throw new NoResultException(
      "회사 정보가 없습니다"
    );
  ...
  public List<CompanyInfo> getCompanyGroup(String companyGroup)
    throws NoResultException {
    List<CompanyInfo> groupList = companyRepository.getCompanyGroupList(
      companyGroup
    );
    if (groupList.size() == 0) throw new NoResultException(
      "회사 정보가 없습니다"
    );
