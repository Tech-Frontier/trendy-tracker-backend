...
public class TechService {
  ...
  public String registTechStack(String tech, TechType type)
    throws AlreadyExistException {
    Boolean isExist = techRepository.isTechRegist(tech).booleanValue();
    if (isExist) throw new AlreadyExistException("해당 기술 스택이 존재합니다");
  ...
  public JSONArray getTechList() {
    ...
    if (techList.size() == 0) throw new NotFoundException(
      "개발 스택 내용이 없습니다"
    );
    ...
    for (Tech tech : techList) {
      ...
      toBeItem.put("type", tech.getType().name());
      toBeList.add(toBeItem);
    }
